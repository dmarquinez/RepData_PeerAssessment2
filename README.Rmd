...
At this moment we are going to rename the EVTYPE to group the same type of events , because a this moment we have ```r length(unique(dataSub$EVTYPE)) ``` different type of events.
...
```
Now we only have ```r length(unique(dataSub$event)) ``` type of events.  

#### Exponents
In the next chunk we are going to calculate the property and crop total damages. We have to calculate it with the ```r names(dataSub)[6:9]``` columns.
```{r , echo = TRUE}
...
For the first question, as we can see in the plot, the top 3 events that were most harmful with respect to population health were ```r dataPeople[1:3,1]``` .
...
For the second question, as we can see in the plot, the top 3 events that were most harmful with respect to population health were ```r dataDamage[1:3,1]``` with a cost in billions of dollars.
